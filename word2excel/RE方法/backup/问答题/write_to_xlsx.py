...
def write_to_xlsx(ques_list,excel_file_path):
    ...
    try:
        with pd.ExcelWriter(excel_file_path, engine='openpyxl', mode='a',if_sheet_exists = 'overlay') as writer: #续写Mode改为'a'
            # #从E(4列)列写入题目
            # new_data.to_excel(writer, sheet_name='Sheet1', startcol=4,startrow=1, index=False, header=False)  
            #从I(8列)列写入答案
            new_data.to_excel(writer, sheet_name='Sheet1', startcol=8,startrow=1, index=False, header=False) 
